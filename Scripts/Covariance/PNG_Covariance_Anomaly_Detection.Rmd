...
# we've manually dropped variables that are colinear and variables that have 0 variance
```{r}
...
site_spread <- mer_png_full %>% select(-HTS_INDEX_NEWPOS, -TX_NET_NEW, -HTS_INDEX_NEWNEG, -HTS_INDEX_KNOWNPOS) # dropping HTS_INDEX_NEWPOS because it has no variance and dropping TX_NET_NEW bc its perfectly colinear with TX_CURR
